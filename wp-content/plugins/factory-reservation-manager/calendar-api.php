<?php
/**
 * Plugin Name: Factory Reservation Calendar API
 * Description: 予約状況カレンダー用のAPIエンドポイント
 * Version: 1.0
 */
 ...
 * REST APIエンドポイントを登録
 */
add_action('rest_api_init', 'register_calendar_api_routes');

function register_calendar_api_routes() {
    register_rest_route('reservation/v1', '/calendar', array(
        'methods' => 'GET',
        'callback' => 'get_calendar_data',
        'permission_callback' => '__return_true',
        'args' => array(
            'month' => array(
                'required' => true,
                'validate_callback' => 'validate_month_parameter',
                'sanitize_callback' => 'sanitize_text_field',
            ),
            'factory' => array(
                'required' => true,
                'validate_callback' => 'validate_factory_parameter',
                'sanitize_callback' => 'sanitize_text_field',
            ),
        ),
    ));
    ...
    register_rest_route('reservation/v1', '/timeslots', array(
        'methods' => 'GET',
        'callback' => 'get_timeslot_data',
        'permission_callback' => '__return_true',
        'args' => array(
            'date' => array(
                'required' => true,
                'validate_callback' => 'validate_date_parameter',
                'sanitize_callback' => 'sanitize_text_field',
            ),
            'factory' => array(
                'required' => true,
 ...
 * 月パラメータの検証
 */
function validate_month_parameter($param, $request, $key) {
 ...
 * 工場IDパラメータの検証
 */
function validate_factory_parameter($param, $request, $key) {
 ...
 * 日付パラメータの検証
 */
function validate_date_parameter($param, $request, $key) {
 ...
 * カレンダーデータを取得
 */
function get_calendar_data($request) {
    global $wpdb;
    ...
    try {
        ...
        // 予約データを取得
        $reservations = get_reservations_for_period($factory_id, $calendar_start, $calendar_end);
        
        // 見学不可日を取得
        $unavailable_days = get_unavailable_days($factory_id, $calendar_start, $calendar_end);
        
        // 工場情報を取得
        $factory_info = get_factory_info($factory_id);
        ...
        while ($current_date <= $calendar_end) {
            ...
            $day_data = array(
                ...
                'am' => calculate_time_slot_status($current_date, 'am', $factory_id, $reservations, $unavailable_days, $is_weekend, $is_holiday),
                'pm' => calculate_time_slot_status($current_date, 'pm', $factory_id, $reservations, $unavailable_days, $is_weekend, $is_holiday),
            );
 ...
 * 時間帯データを取得
 */
function get_timeslot_data($request) {
    $date = $request['date'];
    $factory_id = intval($request['factory']);
    ...
    try {
        ...
        $timeslots = get_available_timeslots($factory_id, $date);
 ...
 * 指定期間の予約を取得
 */
function get_reservations_for_period($factory_id, $start_date, $end_date) {
    global $wpdb;
 ...
 * 見学不可日を取得
 */
function get_unavailable_days($factory_id, $start_date, $end_date) {
    global $wpdb;
 ...
 * 時間帯の状況を計算
 */
function calculate_time_slot_status($date, $time_period, $factory_id, $reservations, $unavailable_days, $is_weekend, $is_holiday) {
    // 土日祝日は見学不可
 ...
 * 利用可能な時間帯を取得
 */
function get_available_timeslots($factory_id, $date) {
 ...
 * 日本の祝日判定（簡易版）
 */
function is_japanese_holiday($date) {
    // TODO: 実際の祝日判定ロジックを実装
    // 現在は常にfalseを返す
    return false;
}
