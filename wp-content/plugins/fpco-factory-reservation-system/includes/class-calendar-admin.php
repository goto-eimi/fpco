...
class FPCO_Calendar_Admin {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_get_factory_info', array($this, 'get_factory_info'));
        add_action('wp_ajax_save_unavailable', array($this, 'save_unavailable'));
        add_action('wp_ajax_get_unavailable_info', array($this, 'get_unavailable_info'));
    }
            ...
            foreach ($factories as $factory) {
                ...
                add_menu_page(
                    ...
                    function() use ($factory) {
                        $this->display_calendar_page($factory->id);
                    },
    ...
    public function enqueue_scripts($hook) {
        ...
        // カスタムスクリプト
        wp_enqueue_script('fpco-calendar-admin', FPCO_RESERVATION_PLUGIN_URL . 'assets/js/calendar-admin.js', array('jquery', 'fullcalendar'), FPCO_RESERVATION_VERSION, true);
        
        // Ajax用のデータを渡す
        wp_localize_script('fpco-calendar-admin', 'fpco_calendar', array(
     ...
     * カレンダー管理画面の表示
     */
    public function display_calendar_page($factory_id = null) {
        global $wpdb;
    ...
    public function get_calendar_events() {
        // nonceチェック
        if (!wp_verify_nonce($_POST['nonce'], 'fpco_calendar_nonce')) {
     ...
     * Ajax: 工場情報を取得
     */
    public function get_factory_info() {
        // nonceチェック
        if (!wp_verify_nonce($_POST['nonce'], 'fpco_calendar_nonce')) {
            wp_die('Security check failed');
        }
     ...
     * Ajax: 見学不可情報を取得
     */
    public function get_unavailable_info() {
        // nonceチェック
        if (!wp_verify_nonce($_POST['nonce'], 'fpco_calendar_nonce')) {
            wp_die('Security check failed');
        }
