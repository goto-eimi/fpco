...
/**
 * 予約管理機能（統合プラグイン版）
 * 元のfactory-reservation-manager/reservation-management.phpから移植
 */
 ...
 * 予約ステータス定数（統合プラグイン版）
 */
if (!defined('FPCO_RESERVATION_STATUS_NEW')) {
 ...
 * 有効なステータス値を取得
 */
function fpco_get_valid_reservation_statuses() {
    return [
 ...
 * ステータス表示名を取得
 */
function fpco_get_reservation_status_label($status) {
    $labels = [
 ...
 * CSSファイルの読み込み
 */
add_action('admin_enqueue_scripts', 'fpco_reservation_management_enqueue_scripts');

function fpco_reservation_management_enqueue_scripts($hook) {
 ...
 * 管理画面メニューを追加
 */
add_action('admin_menu', 'fpco_reservation_management_admin_menu');

function fpco_reservation_management_admin_menu() {
    // 現在のユーザーを取得
    ...
    if ($can_access) {
        add_menu_page(
            ...
            'fpco_reservation_management_admin_page',
            'dashicons-clipboard',
            29  // 工場カレンダーメニューの前に配置
        );
    }
...
function fpco_handle_reservation_form_submission() {
    ...
    // ステータス値の妥当性チェック
    $valid_statuses = fpco_get_valid_reservation_statuses();
 ...
 * バリデーション関数
 */
function fpco_validate_reservation_form($data) {
    $errors = [];
    $field_errors = [];
    ...
    $required_fields = [
        ...
        'applicant_address' => '申込者住所（番地・建物名）',
    ...
    // 予約タイプごとの必須フィールドチェック
    $type_validation = fpco_validate_reservation_type_fields($data);
 ...
 * 予約タイプごとの必須フィールドチェック
 */
function fpco_validate_reservation_type_fields($data) {
    $errors = [];
    $field_errors = [];
 ...
 * 予約タイプをDBのenumに変換
 */
function fpco_get_reservation_type_enum($data) {
    $type_mapping = [
        'school' => 'school',
 ...
 * 予約タイプごとのデータを取得
 */
function fpco_get_type_specific_data($data) {
    $type_data = [];
 ...
 * 予約データをフォームデータに変換
 */
function fpco_convert_reservation_to_form_data($reservation) {
    $form_data = [];
 ...
 * 予約管理画面の表示
 */
function fpco_reservation_management_admin_page() {
    global $wpdb;
    ...
    if (isset($_POST['submit_reservation'])) {
        $result = fpco_handle_reservation_form_submission();
        if ($result['success']) {
            ...
            if ($post_is_edit_mode && $post_reservation_id) {
                ...
                if ($updated_reservation) {
                    $form_data = fpco_convert_reservation_to_form_data($updated_reservation);
                    // 編集モードの状態を維持
                    $is_edit_mode = true;
    ...
    ?>
    <div class="wrap">
        <h1 class="wp-heading-inline"><?php echo $is_edit_mode ? '予約編集' : '予約追加'; ?></h1>
        <?php if ($is_edit_mode): ?>
            <a href="admin.php?page=reservation-management" class="page-title-action">新規追加</a>
        <?php endif; ?>
    ...
    function validateForm() {
        ...
        const requiredFields = [
            ...
            { id: 'applicant_address', name: '申込者住所（番地・建物名）' },
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateReplyEmailButton() {
            if (statusSelect && replyEmailBtn) {
                ...
                if (status === 'pending' && (!isEditMode || savedStatus === 'pending')) {
                    ...
                    replyEmailBtn.style.opacity = '1';
                    replyEmailBtn.style.cursor = 'pointer';
                } else {
                    // その他のステータス：非アクティブ
        ...
        if (replyEmailBtn) {
            replyEmailBtn.addEventListener('click', function() {
                // ボタンが無効な場合は処理を停止
                if (replyEmailBtn.disabled) {
                    alert('返信メールは「確認中」ステータスの予約に対してのみ作成できます。');
                    return;
                }
