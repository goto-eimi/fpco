...
**重要**: 新機能の実装やスキーマ変更が必要な場合は、都度データベース更新の指示を行います。

### データベース更新の手順

1. **事前準備**
   - 必要なテーブルのバックアップを作成
   - `backup_script.sql`を実行してバックアップを取得

2. **マイグレーション実行**
   - `database_migration.sql`を実行してスキーマを更新
   - 新しいテーブルやカラムを追加

3. **確認作業**
   - データベース構造が正しく更新されているか確認
   - 既存データが正常に保持されているか確認

### 注意事項

- **データベース変更は慎重に実行**: 本番環境での実行前に必ず開発環境でテストを行う
- **バックアップの重要性**: 万が一の場合に備えて、変更前に必ずバックアップを取得する
- **段階的な実行**: 大きな変更は複数の小さなステップに分けて実行する
- **ロールバック手順**: 問題が発生した場合の復旧手順を事前に確認する

### 関連ファイル

- `database_migration.sql`: スキーマ更新用のSQLスクリプト
- `backup_script.sql`: バックアップ作成用のSQLスクリプト
- `SPECIFICATION.md`: データベース設計の詳細仕様
